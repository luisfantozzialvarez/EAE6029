...
%				
%				$$ \theta \mathbb{E}\left[\frac{N}{N + \frac{C}{\operatorname{sd}(Z) \operatorname{sd}(V) }}\right]$$
%				onde $\theta = \operatorname{plim}\hat \beta_{MQO} -\beta$ e $N\sim N(0,1)$.
...
%		
%		Mostre que, sob  o regime do item (c) e as hipóteses de homocedasticidade em (d), temos que:
%		
%		 $$\hat t \overset{d}{\to} \frac{C}{ \operatorname{sd}(V)\operatorname{sd}(Z)}+ N$$
%		 onde $N \sim \mathcal{N}(0,1)$.
%		 
%		 \item[e] Usando os itens anteriores, proponha um teste para a hipótese 
\end{itemize}
