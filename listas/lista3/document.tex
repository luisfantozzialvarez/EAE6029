...
\begin{enumerate}
	...
	\item (Monotonicidade condicional) $\mathbb{P}$-quase certamente, temos que $\mathbb{P}[Z(1)\geq Z(0)|X]\in \{0,1\}$.
	\item (Primeiro estágio condicional)  $\mathbb{P}[\mathbb{E}[Z(1)|X]-\mathbb{E}[Z(0)|X]]>0$.
\end{enumerate}
...
\begin{enumerate}
	...
	\item[b] Seja $\boldsymbol{p}(X) = \begin{bmatrix}
		...
		\vdots \\
		\mathbf{1}_{\{x_L\}}(X) 
	\end{bmatrix}$, um vetor com ``dummmies'' indicadoras dos pontos de suporte de $X$. Mostre que o estimador de MQO em dois estágios de $\beta$ no sistema linear:
	...
	$$Y = \beta D + \gamma' \boldsymbol{p}(X) + U\, , $$
		...
		$$\tau_{LATE}(X) = \frac{\mathbb{E}[(Y(1)-Y(0))\mathbf{1}_{Z(1)\neq Z(0)}|X]}{\mathbb{P}[Z(1)\neq Z(0)|X]}$$
		e $w(X)$ são pesos tais que $w(X) \geq 0$ quase certamente, e $\mathbb{E}[w(X)]$. Para quais subpopulações definidas pelos valores de $X$ o estimando atribui maior peso ao efeito causal?
\end{enumerate}
