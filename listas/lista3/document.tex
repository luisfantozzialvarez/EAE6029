...
\paragraph{Exercício 5 (\textit{Eminent Domain})} Neste exercício, vamos estudar o impacto do \emph{eminent domain} sobre os preços de imóveis nos Estados Unidos\footnote{Este exercício é baseado em \citet{chen2015growth}.}. \emph{Eminent domain} (ou leis de desapropriação) é o mecanismo legal pelo qual o governo pode adquirir compulsoriamente uma propriedade privada. Esse tipo de ação é bastante controverso, e os proprietários podem recorrer à Justiça para tentar reverter a decisão. Quando tribunais federais decidem que uma desapropriação foi ilegal (\emph{decisões favoráveis ao autor}, \emph{pro-plaintiff}), os direitos de propriedade são reforçados e cria-se um precedente que torna mais difícil a utilização futura do \emph{eminent domain}. Nosso objetivo será analisar o efeito causal dessas decisões favoráveis ao autor, proferidas nos tribunais federais de apelação, sobre os preços de imóveis.\footnote{``Uma compreensão básica do funcionamento dos tribunais federais dos EUA é importante para entender nossa estratégia de identificação, que se baseia na função de criação de precedentes dos tribunais de \emph{common law}. As decisões judiciais em casos atuais se tornam precedentes que podem ser utilizados em casos futuros no mesmo tribunal e em tribunais inferiores da mesma jurisdição. Existem três níveis de tribunais federais: Distrito, Circuito e a Suprema Corte dos EUA. Os 94 Tribunais Distritais funcionam como tribunais de primeira instância, onde um júri decide \emph{questões de fato}. Caso uma das partes recorra da decisão, o caso segue para um Tribunal de Circuito, que decide \emph{questões de direito}; esses tribunais tomam os fatos estabelecidos no Tribunal Distrital como dados e não utilizam júri. Os 12 Tribunais de Circuito, também conhecidos como Tribunais de Apelação, analisam apenas casos que levantam novas questões jurídicas (apenas 10--20\% das decisões dos tribunais distritais são objeto de recurso). Os casos que chegam aos tribunais de circuito costumam ser os mais complexos e controversos, com maior probabilidade de estabelecer novos precedentes.'' (Chen e Yeh, p. 11--12).} 

\noindent \subparagraph{a} Importe \texttt{temp\_CSHomePrice.mat.csv}, disponível na subpasta \texttt{dados}. Esse conjunto de dados contém informações sobre preços de imóveis (índice Case-Shiller) e decisões favoráveis ao autor por circuito judicial e período. Observe que, com exceção do índice Case-Shiller em log, todas as variáveis estão em frequência anual. Agregue os dados por ano, calculando a média \emph{dentro de cada circuito-ano} do log do índice de preços.

\noindent \subparagraph{b} Estime uma especificação linear por MQO do log do índice de preços em um intercepto e  no número total de decisões de desapropriação em apelação favoráveis ao autor (\texttt{numpro\_casecat\_12}). O que você encontra? Esse coeficiente pode ser interpretado como uma estimativa de um efeito causal? Por quê?
...
\noindent \subparagraph{c} Faça o \emph{merge} do conjunto de dados \texttt{probs.txt} com sua base. Esse arquivo contém a distribuição das características dos juízes em cada circuito entre 1945 e 2007.

\noindent \subparagraph{d} Considere o seguinte modelo linear causal:
...
\begin{enumerate}
	...
	\item (Monotonicidade condicional) $\mathbb{P}$-quase certamente, temos que $\mathbb{P}[D(1)\geq D(0)|X]\mathbb{P}[D(1)\leq D(0)|X]=0$ e $\mathbb{P}[D(1)\geq D(0)|X]+\mathbb{P}[D(1)\leq D(0)|X]=1$.
	\item (Primeiro estágio condicional)  $\mathbb{P}[\mathbb{E}[D(1)|X] - \mathbb{E}[D(0)|X] \neq 0]>0$.
\end{enumerate}
...
Um colega, entretanto, tendo lido um artigo recentemente \citep{blandhol2022tsls}, nota que, nesse caso, para obter uma interpretação causal sob heterogeneidade irrestrita dos efeitos de tratamento, requererá modificar 3 e 4, para.
...
\begin{itemize}
	...
	\item[d] Encontrei a interpretação causal do estimando de 2SLS do novo sistema em termos de uma média poderadas de LATEs condicionais. Como os pesos diferem da representação do estimando em (b)?
