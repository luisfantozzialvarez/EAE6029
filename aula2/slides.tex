...
\end{itemize}
\vspace{0.8em}
		$$\footnotesize W_{R,q} = \frac{(R\hat{b}-c)'\left(R\hat{\sigma}^2(\boldsymbol{X}'\boldsymbol{X})^{-1}R'\right)^{-1}(R\hat{b}-c)}{k}  = \frac{(R\hat{b}-c)'\left(R(\boldsymbol{X}'\boldsymbol{X})^{-1}R'\right)^{-1}(R\hat{b}-c)}{k \hat{\sigma}^2}\, ,$$
...
\begin{proposition}
	...
	$$W_{R,q}|\boldsymbol{X}\sim F(k,n-k)$$
\end{proposition}
\begin{itemize}
