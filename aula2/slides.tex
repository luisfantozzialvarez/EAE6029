...
\end{transitionframe}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \textbf{Exemplo:} se $Y_i$ é a taxa de inflação no período $i$, e $X_i$ a taxa de desemprego no período $i-1$, analisaremos as propriedades dos estimadores sob realizações alternativas do cenário econômico em que a taxa de desemprego é igual à observada nos períodos $i=0,\ldots,n-1$.  
		\end{itemize}
...
\begin{itemize}
		...
		\begin{itemize}
			\item A esperança condicional $\mathbb{E}[Y|X]$ das variáveis $(Y,X)$ que representam a distribuição das características na população de interesse é linear em $X$, {\color{green}e coincide com o modelo linear de interesse}.
		...
		\end{itemize}
					...
					\begin{itemize}
						\item Por exemplo, se as observações estão ordenadas no tempo e o modelo postulado é causal, $\mathbb{E}[\epsilon_i|\boldsymbol{X}]=0$ implica que causas não observadas  do fenômeno no período $i$ não exibem associação sistemática com as causas observadas $X$ em $i$ {\color{red}e em nenhum outro período}.
						\begin{itemize}
...
\begin{frame}{Homocedasticidade}
	...
	\begin{itemize}
		...
		\end{itemize}
\item Sob H1-H3, variância do estimador de MQO é dada por: $\mathbb{V}[\hat{b}|\boldsymbol{X}]= \sigma^2 (\boldsymbol{X}'\boldsymbol{X})^{-1}$.
	\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	$$\phi \coloneqq \mathbf{1}\{W_{R,c}> q_F(1-\alpha|r,n-k)\}$$
	onde $q_F(1-\alpha|r,n-k)$ é o quantil $1-\alpha$ da distribuição $F(r,n-k)$.
	\begin{itemize}
	...
	\end{itemize}
	\item No caso em que $r=1$, teste é equivalente a rejeitar a nula se:
	...
	\vspace{-0.5em}
	$$\hat{t}_{R,c} = \frac{(R\hat{\beta}-c)}{\sqrt{R\hat{\sigma}^2(\boldsymbol{X}'\boldsymbol{X})^{-1}R'}}$$
\end{itemize}
\end{frame}
\begin{transitionframe}
	\centering
...
\begin{frame}{Regime Assintótico}
	\begin{itemize}
		...
		\end{itemize}
		\item Para isso, focaremos no caso em que observamo++s uma amostra aleatória de uma população cuja distribuição é representada pelo para de variáveis aleatórias $(X,Y)$.
		\begin{itemize}
